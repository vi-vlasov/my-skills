...
const wall = new THREE.Mesh(new THREE.PlaneGeometry(7.2, 4.05), new THREE.MeshBasicMaterial({ map: wallTexture() }));
wall.position.set(0, 1.5, -2.2);
scene.add(wall);
const halo = new THREE.Mesh(new THREE.PlaneGeometry(2.65, 2.65), new THREE.MeshBasicMaterial({
    map: haloTexture(),
    transparent: true,
    opacity: 0.65,
    toneMapped: false,
...
scene.add(halo);
const lowerWash = new THREE.Mesh(new THREE.PlaneGeometry(7.8, 1.9), new THREE.MeshBasicMaterial({
...
scene.add(lowerWash);
const menuGlow = new THREE.Mesh(new THREE.PlaneGeometry(7.8, 0.32), new THREE.MeshBasicMaterial({
...
function glowStrip(x, w, intensity, opacity = 0.82) {
    const m = new THREE.Mesh(new THREE.PlaneGeometry(w, 4.2), new THREE.MeshBasicMaterial({
...
function onProgress(xhr) {
    if (xhr.total)
        bootBar.style.width = `${Math.round((xhr.loaded / xhr.total) * 100)}%`;
...
function setBoneRotationDeltas(rig, bones, x = 0, y = 0, z = 0) {
    for (const bone of bones)
        setBoneRotationDelta(rig, bone, x, y, z);
}
...
function setBonePositionDeltas(rig, bones, x = 0, y = 0, z = 0) {
    for (const bone of bones)
        setBonePositionDelta(rig, bone, x, y, z);
}
const BLINK_NATIVE = {
    restClose: 0.12,
    maxClose: 0.82,
...
function getNativeBlinkClose(blink) {
    ...
    return THREE.MathUtils.clamp(BLINK_NATIVE.restClose + animatedClose * (BLINK_NATIVE.maxClose - BLINK_NATIVE.restClose), BLINK_NATIVE.restClose, BLINK_NATIVE.maxClose);
}
...
function applyEyeBlinkVisibility(eyeMats, close) {
    ...
    for (const mat of eyeMats) {
        if (!mat.userData.openBlinkColor)
            mat.userData.openBlinkColor = mat.color.clone();
        if (mat.userData.openBlinkEnv == null)
            mat.userData.openBlinkEnv = mat.envMapIntensity ?? 0;
...
function applyLashBlinkVisibility(lashMats, close) {
    ...
    for (const mat of lashMats) {
        if (mat.userData.openBlinkOpacity == null)
            mat.userData.openBlinkOpacity = mat.opacity ?? 1;
...
function chloeMaterials() {
    ...
    const skin = (alb, nrm, rough) => new THREE.MeshPhysicalMaterial({
        map: tex(A + alb, true),
        normalMap: tex(A + nrm),
    ...
    const cutout = (map, opts = {}) => new THREE.MeshStandardMaterial({
        map: tex(A + map, true),
        transparent: true,
        alphaTest: 0.32,
...
function setupChloe(gltf) {
    ...
    model.traverse((obj) => {
        ...
        if (name.startsWith('Chloe_Duct'))
            obj.material = mats.Chloe_Duct;
        else if (mats[name])
            obj.material = mats[name];
        obj.material.vertexColors = false;
        if (name === 'Chloe_Eyes')
            headGroup.userData.eyeMats = [obj.material];
        if (name === 'Chloe_Lashes')
            headGroup.userData.lashMats = [obj.material];
        if (name === 'Chloe_Circle')
            headGroup.userData.ledMats = [obj.material];
    });
...
function setupFacecap(gltf) {
    ...
    raycaster.set(new THREE.Vector3(-0.9, 1.49 + 0.11, 0.85), new THREE.Vector3(0.72, -0.04, -0.75).normalize());
...
gltfLoader.load('assets/chloe/chloe.glb', (gltf) => { setupChloe(gltf); modelReady(); }, onProgress, () => {
    gltfLoader.load('assets/facecap.glb', (gltf) => { setupFacecap(gltf); modelReady(); }, onProgress, (err) => {
...
const bloom = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 0.34, // strength — halo bleeding over the silhouette edges
0.74, // radius
...
function setBlinkDebugMode(mode) {
    if (!['animate', 'open', 'half', 'closed'].includes(mode))
        return;
...
window.addEventListener('keydown', (e) => {
    if (e.altKey) {
        if (e.code === 'Digit0')
            setBlinkDebugMode('animate');
        else if (e.code === 'Digit1')
            setBlinkDebugMode('open');
        else if (e.code === 'Digit2')
            setBlinkDebugMode('half');
        else if (e.code === 'Digit3')
            setBlinkDebugMode('closed');
        else if (e.code === 'KeyB') {
            living.blinkDebugMode = 'animate';
    ...
    if (e.key === 'ArrowRight')
        ...
        openPanel(items[selected].dataset.panel);
});
...
function animate() {
    ...
    motion.lookX = THREE.MathUtils.damp(motion.lookX, pose.lookX + interfaceBiasX + motion.viewerX * 0.12, 4.5, dt);
    motion.lookY = THREE.MathUtils.damp(motion.lookY, pose.lookY + interfaceBiasY - motion.viewerY * 0.08, 4.5, dt);
    ...
    if (rig) {
        ...
        setBoneRotationDelta(rig, rig.eyeL, -attentiveLookY * 0.55, attentiveLookX * -1.75, -living.eyeDriftX * 0.015);
        setBoneRotationDelta(rig, rig.eyeR, -attentiveLookY * 0.55, attentiveLookX * -1.75, living.eyeDriftX * 0.015);
