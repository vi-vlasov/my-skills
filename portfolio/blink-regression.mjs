...
assert.match(source, /const cornerUpL = -0\.000048 \* liftL/, 'mouth corners should lift enough for the warm reference smile');
assert.match(source, /const cornerOutL = 0\.00001 \* liftL/, 'mouth corners should widen the expression without exposing side teeth');
assert.match(source, /const upperLift = -0\.000072 \* smileLift - 0\.000082 \* open/, 'upper lip should lift enough for a readable soft smile');
assert.match(source, /const lowerDrop = 0\.000145 \* open/, 'center lower lip should drop enough for a visible central smile aperture');
assert.match(source, /const sideSeal = 0\.000041 \* open/, 'side lips should seal the corners so teeth stay centered');
assert.match(source, /lowerDrop \* 0\.44 - sideSeal/, 'side lower lips should follow the center enough to avoid a puppet V-mouth');
assert.match(source, /const smile = THREE\.MathUtils\.clamp\(0\.34 \+ expressiveMouth \* 0\.26/, 'menu expression should keep a warmer reference-like smile');
