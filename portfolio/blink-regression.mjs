...
assert.match(source, /const cornerUpL = -0\.000058 \* liftL/, 'mouth corners should lift enough for the warm reference smile');
assert.match(source, /const cornerOutL = 0\.000011 \* liftL/, 'mouth corners should widen the expression without exposing side teeth');
assert.match(source, /const cheekLiftL = -0\.000052 \* liftL/, 'cheeks should lift enough to make the smile feel alive');
assert.match(source, /const upperLift = -0\.00011 \* smileLift - 0\.000148 \* open/, 'upper lip should lift enough for a readable speaking smile');
assert.match(source, /const lowerDrop = 0\.000112 \* open/, 'center lower lip should avoid a puppet V while keeping the central smile aperture');
assert.match(source, /const sideSeal = 0\.000058 \* open/, 'side lips should seal the corners so teeth stay centered');
assert.match(source, /lowerDrop \* 0\.46 - sideSeal/, 'side lower lips should follow enough for a horizontal speaking smile without fang-like side teeth');
assert.match(source, /const smile = THREE\.MathUtils\.clamp\(0\.38 \+ expressiveMouth \* 0\.28/, 'menu expression should keep a warmer reference-like smile');
assert.match(source, /const mouthOpen = THREE\.MathUtils\.clamp\(0\.18 \+ expressiveMouth \* 0\.3/, 'menu expression should keep a reference-like soft open mouth without fangy teeth');
assert.match(source, /const jawOpen = 0\.014 \+ mouthOpen \* 0\.19/, 'jaw opening should stay soft but visible for a relaxed DBH speaking expression');
assert.match(source, /setBoneRotationDeltas\(rig, rig\.upperLip, -mouthOpen \* 0\.078/, 'upper lip should lift enough to reveal a central speaking-smile line');
...
assert.match(source, /closedEyeOpacity: 0\.14/, 'closed eyes should hide glassy iris reflections under the eyelid');
...
assert.match(source, /living\.eyeDriftX = THREE\.MathUtils\.damp\(living\.eyeDriftX, living\.eyeTargetX, 1\.12, dt\)/, 'gaze drift should move calmly rather than twitching');
...
assert.ok(blinkClosing >= 0.07 && blinkClosing <= 0.085, 'blink should snap closed quickly enough to read as intentional instead of twitchy');
assert.ok(blinkClosed >= 0.32, 'closed blink state should be held long enough for screenshots and perception');
assert.ok(blinkOpening >= 0.22 && blinkOpening <= 0.25, 'blink opening should be calm but not so slow that the eye looks like it is drifting');
assert.ok(defaultBlinkSpeed >= 1.14 && defaultBlinkSpeed <= 1.2, 'default blink speed should be forceable without erasing the closed hold');
